...
<!-- directly learns a parameterized policy $$\pi_{\theta}$$, whereas other reinforcement learning methods only learn state-action value function, and the policy is typically obtained by greedily acting on the value function (by simply choosing the action whose value is the highest).   -->
<!-- In short, -->

<!-- <div> -->
<!-- $$ -->
<!-- \begin{align} -->
<!-- \text{value based:} &&~~a &= \text{arg}\max_{a'}{Q(s, a')}\\ -->
<!-- \text{policy gradient:} &&~~ a &\sim \pi_\theta(\cdot | s) \\ -->
<!-- \end{align} -->
<!-- $$ -->
<!-- </div> -->
...
<!-- # Example: REINFORCE -->
<!-- REINFORCE is one of the simplest algorithms based on policy gradient. -->
<!-- The idea is to approximate the expectation in equation (3) by samples from the trajectory following $$\pi_\theta$$. -->
<!-- And state-action value $$Q^{\pi_{\theta}}$$ by a sample return $$G_t$$. -->

<!-- Given a trajectory, the gradient is calculated for each sample $$(s_t, a_t, r_{t+1})$$ as -->
<!-- $$ -->
<!-- \begin{align} -->
<!-- \hat{\nabla J(\theta)} &= \color{brown}\gamma^{\color{brown}t} \cdot \frac{\nabla \pi_{\theta}(a_t|s_t)}{\pi_{\theta}(a_t|s_t)} \cdot G_t \\ -->
<!-- \text{, where} ~~G_t &= \sum_{t'= t + 1}^T \gamma^{t'- t - 1} r_{t'}. -->
<!-- \end{align} -->
<!-- $$ -->

<!-- It should be noted that $$\color{brown}\gamma^t$$ is blah... -->
<!-- As a reference, the complete algorithm is shown below. -->
